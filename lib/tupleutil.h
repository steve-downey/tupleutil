...
constexpr
std::array<std::variant<Args...>, sizeof...(Args)>
...
template <typename Func, typename Tuple, std::size_t... I>
auto tuple_transform_impl(Tuple && tuple,
                          Func&&       f,
                          std::index_sequence<I...>) {
    return std::make_tuple(std::forward<Func>(f)(std::get<I>(std::forward<Tuple>(tuple)))...);
}
...
{
    return [](T const& t) {
        return V{std::in_place_index_t<I>{},
                std::get<I>(t)};
    };
}
...
template <typename... Args>
auto tuple_getters(std::tuple<Args...>) {
